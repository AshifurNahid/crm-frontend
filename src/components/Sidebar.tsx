...
const Sidebar = ({ activeModule, onModuleChange }: SidebarProps) => {
  ...
  const handleModuleClick = (module: any) => {
    if (module.route) {
      ...
      // For modules without dedicated routes, stay on current page and change module
      if (location.pathname === '/') {
        onModuleChange(module.id);
      } else {
        navigate('/');
        setTimeout(() => onModuleChange(module.id), 100);
      }
    }
  };

  const getActiveModule = () => {
    if (location.pathname === '/territories') return 'territories';
    return activeModule;
  };
